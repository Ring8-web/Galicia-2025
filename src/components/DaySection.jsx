  ...
  return (
    <section id={id} className="max-w-7xl mx-auto px-4 md:px-6 mt-16 pb-16 print:break-before-page">
      ...
      <section id={`${id}-mapa`} className="mt-8">
        ...
        <img src={`/fotos/${id}-mapa.jpg?v=${version}`} alt={`Mapa ${titulo}`} className="mt-3 w-full rounded-lg" onError={(e)=>{e.currentTarget.style.display='none'}}/>
        <p className="text-xs text-slate-500 mt-2">Coloca el archivo en <code>/public/fotos/{id}-mapa.jpg</code></p>
      </section>
      ...
      <section id={`${id}-galeria`} className="mt-10">
        ...
        <p className="text-sm text-slate-500 mt-1">Fotos desde <code>/fotos/</code>, con enlace de descarga por imagen.</p>
