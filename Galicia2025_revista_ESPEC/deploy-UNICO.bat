...
set "COMMIT_MSG=Deploy automatizado (build + fotos + vercel)"
...
where vercel>nul 2>nul || (echo ⚠️ Instalando Vercel CLI... & npm i -g vercel >nul 2>&1)
...
echo 🔧 Compilando...
npm run build || (echo ❌ Error en la compilacion. & pause & exit /b 1)
...
echo 🔗 Vercel link (solo primera vez si hace falta)...
